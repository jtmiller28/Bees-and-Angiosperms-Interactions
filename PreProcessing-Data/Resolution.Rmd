...
# Defunct! It appears that this is not in fact rigirous enough as an umbrella resolution. 
dsf_res_angios_f <- dsf_res_angios %>% 
  filter(Fuzzy.dist <= 3)
...
dsf_res_angios_ff <- dsf_res_angios_f %>% 
  filter(!taxonomicStatus == "") # We cant have records with blank relations..
...
```
### Dealing with "Doubtful" Names. 
Currently Nomer does not recongize the designation of Doubtful in WFO. [Currently waiting on definitive definition of these name relations meaning, however; it seems prudent to create a table that completely removes these types of names]
For now, lets make 2 versions of the final table to check for name alignment. 
```{r}
### Locate the names in WFO online's backbone
wfo_backbone <- read.delim("/home/jt-miller/Globi-Bees-Plant-Interactions/WFO/2022.07-Backbone/classification.txt", quote = "")

# Isolate instances of "Doubtful"
wfo_backbone_doubtful <- wfo_backbone %>% 
  filter(taxonomicStatus == "DOUBTFUL")



# Create a vector of these names for pulling out of our data...
wfo_doubtfuls <- wfo_backbone_doubtful$scientificName

# Create a version where final name table is original
final_name_table <- final_name_table 

# Create a version where "doubtful" names are removed
final_name_table2 <- final_name_table %>% 
  filter(!acceptedNames %in% wfo_doubtfuls) # Cuts almost 1300 names out of the final mapping table. 
...
write.csv(final_name_table2, "/home/jt-miller/Globi-Bees-Plant-Interactions/Plant-Data/Nomer-wfo-resolution/Final-Resolution/final_name_table_w_out_doubtful.csv", row.names = FALSE)
```
...
### Repeat for Doubtfuless data designations. 
```{r}
# Read in mapping table...
final_name_table <- read.csv("/home/jt-miller/Globi-Bees-Plant-Interactions/Plant-Data/Nomer-wfo-resolution/Final-Resolution/final_name_table_w_out_doubtful.csv")
...
# Write it out to a txt and csv -> Send it to another script to join with post-processed data
```{r}
write.csv(full_res_df, "/home/jt-miller/Globi-Bees-Plant-Interactions/Plant-Data/Nomer-wfo-resolution/Final-Resolution/Resolved_verbatim_w_out_doubtful.csv", row.names = FALSE)

write.table(full_res_df, file = "/home/jt-miller/Globi-Bees-Plant-Interactions/Plant-Data/Nomer-wfo-resolution/Final-Resolution/Resolved_verbatim_w_out_doubtful.txt", row.names = FALSE, sep="\t", quote = FALSE)
```
